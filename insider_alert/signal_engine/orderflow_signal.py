...
_COMPONENTS = [
    SignalComponent(
        ...
        abs_value=True, flag_template="High bid-ask imbalance: {value:.2f}",
    ),
    SignalComponent(
        key="aggressive_buy_ratio", max_score=25, normaliser=1.0,
        flag_template="Aggressive buying: {value:.2f}",
    ),
    SignalComponent(
...
def compute_orderflow_anomaly_signal(features: dict) -> dict:
    ...
    return compute_signal("orderflow_anomaly", features, _COMPONENTS)

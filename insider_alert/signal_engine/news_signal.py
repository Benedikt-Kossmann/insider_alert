...
# Scoring constants for the news divergence signal
_DIVERGENCE_NORMALISER = 0.05   # 5 % unexplained move maps to full divergence score
_DIVERGENCE_MAX_SCORE = 60      # maximum score points from the price/news divergence
_SENTIMENT_MAX_SCORE = 40       # maximum score points from the sentiment conflict
...
def compute_news_divergence_signal(features: dict) -> dict:
    ...
    if news_count > 0:
        sentiment_conflict = (1 - abs(sentiment)) * _SENTIMENT_MAX_SCORE
    else:
        sentiment_conflict = 0.0

    if divergence_component > _DIVERGENCE_MAX_SCORE / 2:
